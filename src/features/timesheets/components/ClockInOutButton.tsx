...
export function ClockInOutButton() {
  ...
  const { data: activeShift, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await db.query<TimesheetShift>(
        `SELECT id FROM timesheets WHERE user_id = $1 AND clock_out_time IS NULL AND is_deleted = false ORDER BY clock_in_time DESC LIMIT 1`,
        [currentUserId]
      );
  ...
  const clockInMutation = useMutation({
    mutationFn: async () => {
      ...
      await db.query(
        `INSERT INTO timesheets (user_id, shift_date, clock_in_time, status, created_by, modified_by) VALUES ($1, CURRENT_DATE, now(), 'CLOCKED_IN', $1, $1)`,
        [currentUserId]
      );
    },
    onSuccess: () => {
  ...
  const clockOutMutation = useMutation({
    mutationFn: async (shiftId: string) => {
      if (!currentUserId) throw new Error("No user ID");
      await db.query(
        `UPDATE timesheets SET clock_out_time = now(), status = 'CLOCKED_OUT', modified_by = $1, updated_at = now() WHERE id = $2`,
