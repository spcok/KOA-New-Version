...
import { 
  ...
  HeartPulse, AlertTriangle, Flame, Loader2, ShieldCheck, Wifi, WifiOff, RefreshCw, LogOut
} from 'lucide-react';
...
// Export as rootRoute for child routes to import
...
function RootComponent() {
  ...
  // 1. DATABASE BOOT: Await the PGlite Clinical Vault
  useEffect(() => {
    async function boot() {
      try {
        await db.waitReady;
  ...
  // 2. SYNC ENGINE: Start only if authenticated
  useEffect(() => {
  ...
  const ActiveLink = ({ to, icon, label }: { to: string; icon: React.ReactNode; label: string }) => (
    <Link 
      ...
      inactiveProps={{ className: "text-slate-400 hover:text-white hover:bg-slate-800/50" }} 
  ...
  return (
    <div className="flex h-screen w-full bg-[#0A0B0E] font-sans text-slate-300 overflow-hidden">
      ...
      <aside className={`${isSidebarOpen ? 'w-64' : 'w-20'} flex-shrink-0 flex flex-col border-r border-slate-800/60 transition-all duration-300 bg-[#0A0B0E]`}>
        <div className="flex items-center justify-center h-16 px-4 bg-[#0A0B0E] border-b border-slate-800/40">
        ...
        <nav className="flex-1 py-6 overflow-y-auto px-3 space-y-8 scrollbar-hide">
          <section>
            {isSidebarOpen && <label className="px-4 text-[10px] font-black tracking-[0.2em] text-slate-600 uppercase block mb-3">Core</label>}
            <ActiveLink to="/" label="Dashboard" icon={<Activity size={20} />} />
          </section>

          <section>
            {isSidebarOpen && <label className="px-4 text-[10px] font-black tracking-[0.2em] text-slate-600 uppercase block mb-3">Clinical</label>}
            <div className="space-y-1">
              <ActiveLink to="/medical" label="Medical" icon={<HeartPulse size={20} />} />
